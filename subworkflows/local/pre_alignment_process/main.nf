...
workflow PRE_ALIGNMENT_PROCESSING {
    ...
    val_refToAdd // file: the reference that should be added
    ...
    if (val_addRefToSeqs) {
        ...
        ADD_SEQUENCES(
            COLLAPSE.out.sample_tuple.merge(TRANSLATE_REFERENCE.out.sample_tuple) { sample, ref -> tuple([sample[0], ref[0]], sample[1]) }
        )
